...
def run_formal_test(name: str, product_name: str, hand: str) -> dict[str, Any]:
    normalized_inputs = [value.strip() for value in (name, product_name, hand)]
    if not all(normalized_inputs):
        raise RuntimeError("name、product_name、hand 都不能为空")
    normalized_name, normalized_product_name, normalized_hand = normalized_inputs
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="只输入 name、product_name、hand，自动匹配本地图片并测试正式 Locate API"
    )
    parser.add_argument("name", help="请求的 name 字段")
...
def main_cli() -> None:
    args = parse_args()
    try:
        result = run_formal_test(args.name, args.product_name, args.hand)
