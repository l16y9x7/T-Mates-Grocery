...
当前 `StatusResponse` 严格要求只有 `status=SUCCEEDED`，因此收到上述扩展响应会在 8086 本地解析失败。并且 `simulated=true` 表示没有实际驱动机器人，不能当作真实抓取完成。
...
这表示服务处于模拟模式，并不代表机械臂真的完成了抓取。当前 8086 还严格期待 `{"status":"SUCCEEDED"}`，因此需要在执行响应适配中兼容真实服务的扩展响应，并明确区分 `simulated=true` 与真实动作成功。
